...
namespace Jakaria.API
{
    ...
    public class WaterModAPI : MySessionComponentBase
    {
        ...
        private static Func<MyPlanet, float, bool> _CreateWater;
        private static Func<MyPlanet, float, bool> _SetWaterRadius;
        private static Func<Action<MyPlanet>, bool> _RegisterOnWaterChanged;
        private static Func<Action<MyPlanet>, bool> _UnregisterOnWaterChanged;
        ...
        /// <summary>
        /// Creates water on a planet. Does not check permission, must be called Server-side only.
        /// for client side call, use <see cref="RunCommand"/>
        /// </summary>
        /// <param name="Radius">Relative to the planet minimum radius.</param>
        public static bool CreateWater(MyPlanet planet, float Radius = 1f) => _CreateWater?.Invoke(planet, Radius) ?? false;

        /// <summary>
        /// Sets the radius of existing water. Does not check permission, must be called Server-side only.
        /// for client side call, use <see cref="RunCommand"/>
        /// </summary>
        /// <param name="Radius">Relative to the planet minimum radius.</param>
        ...
        /// <summary>
        /// Registers a callback invoked whenever a water sync packet for a planet is processed locally.
        /// Works on both server and clients. The callback receives the affected planet.
        /// </summary>
        public static bool RegisterOnWaterChanged(Action<MyPlanet> callback) => _RegisterOnWaterChanged?.Invoke(callback) ?? false;

        /// <summary>
        /// Unregisters a callback previously registered with <see cref="RegisterOnWaterChanged"/>.
        /// </summary>
        public static bool UnregisterOnWaterChanged(Action<MyPlanet> callback) => _UnregisterOnWaterChanged?.Invoke(callback) ?? false;
            ...
            if (obj is Dictionary<string, Delegate>)
            {
                ...
                if (Registered)
                {
                    try
                    {
                        ...
                        _CreateWater = TryGetMethod<Func<MyPlanet, float, bool>>(ModAPIMethods, "CreateWater");
                        _SetWaterRadius = TryGetMethod<Func<MyPlanet, float, bool>>(ModAPIMethods, "SetWaterRadius");
                        _RegisterOnWaterChanged = TryGetMethod<Func<Action<MyPlanet>, bool>>(ModAPIMethods, "RegisterOnWaterChanged");
                        _UnregisterOnWaterChanged = TryGetMethod<Func<Action<MyPlanet>, bool>>(ModAPIMethods, "UnregisterOnWaterChanged");
    }
                    catch (Exception e)
                    {
